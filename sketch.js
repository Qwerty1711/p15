...
function draw() {
  ...
  if(path.y > 400 ){
    path.y = height/3.5; //fixed the background animation
  }
...
function createCash() {
  ...
  cash.scale=0.12;
  cash.velocityY = 4; //made the items move with the floor instead of lagging behind
  cash.lifetime = 150;
  cashG.add(cash);
  }
}
...
function createDiamonds() {
  ...
  diamonds.scale=0.03;
  diamonds.velocityY = 4; //made the items move with the floor instead of lagging behind
...
function createjewelry() {
  ...
  jewelry.scale=0.13;
  jewelry.velocityY = 4; //made the items move with the floor instead of lagging behind
...
function createSword(){
  if (World.frameCount % Math.round(random(300, 500)) == 0) {
  var sword = createSprite(boy.x ,40, 10, 10);
  sword.addImage(swordImg);
  sword.scale=0.1;
  sword.velocityY = 30; ////made the swords move as a projectile to increase difficulity
  sword.lifetime = 150;
